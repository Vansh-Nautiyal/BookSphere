...
public class OperatorPanel extends JPanel {
    ...
    public OperatorPanel(User currentUser) {
    ...
    private JPanel buildToolbar() {
        ...
        if (currentUser.getRole().equals("admin") || currentUser.getRole().equals("operator")) {
    ...
    private JScrollPane buildTable() {
        ...
        if (currentUser.getRole().equals("admin")) {
            ...
            JMenuItem edit = mi("Edit");
            edit.addActionListener(e -> editSelected());
            JMenuItem act = mi("Activate");
            act.addActionListener(e -> setStatus("active"));
            JMenuItem sus = mi("Suspend");
            sus.addActionListener(e -> setStatus("suspended"));
            JMenuItem del = mi("Delete");
    ...
    public void loadMembers() {
        ...
        for (User u : UserDAO.getAllUsers()) {
            if (u.getRole().equals("admin"))
                continue;
            if (currentUser.getRole().equals("operator") && u.getRole().equals("operator"))
                continue;
    ...
    private void showDialog(User existing) {
        ...
        dlg.setSize(430, isEdit ? 460 : 560);
        ...
        if (currentUser.getRole().equals("admin")) {
            role = new JComboBox<>(new String[] { "student", "operator" });
        } else {
            role = new JComboBox<>(new String[] { "student" });
        }
        if (isEdit && existing.getRole().equals("operator")
        ...
        if (!isEdit) {
            ...
            pass.setBackground(BG_DARK);
            pass.setCaretColor(WHITE);
        ...
        save.addActionListener(e -> {
            if (gid.getText().trim().isEmpty() || name.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(dlg, "Global ID and Name required.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (!isEdit && (fPass == null || new String(fPass.getPassword()).trim().isEmpty())) {
                JOptionPane.showMessageDialog(dlg, "Password required.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (!isEdit && UserDAO.globalIdExists(gid.getText().trim())) {
                JOptionPane.showMessageDialog(dlg, "Global ID already in use.", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (!isEdit) {
                if (UserDAO.register(u)) {
                    ...
                    JOptionPane.showMessageDialog(dlg, "Failed.", "Error", JOptionPane.ERROR_MESSAGE);
            } else if (UserDAO.updateUser(u)) {
                loadMembers();
                dlg.dispose();
            } else
                JOptionPane.showMessageDialog(dlg, "Failed to update member.", "Error", JOptionPane.ERROR_MESSAGE);
        });
        p.add(save);
    ...
    private void editSelected() {
        ...
        if (role.equals("operator") && !currentUser.getRole().equals("admin")) {
            JOptionPane.showMessageDialog(this,
                    "You cannot edit another operator.",
                    "Access Denied",
    ...
    private void setStatus(String status) {
        ...
        if (row < 0)
            return;
        if (UserDAO.updateStatus((int) model.getValueAt(row, 0), status))
            loadMembers();
    }
    ...
    private void deleteSelected() {
        ...
        if (role.equals("operator") && !currentUser.getRole().equals("admin")) {
            JOptionPane.showMessageDialog(this,
                    "You cannot remove another operator.",
                    "Access Denied",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private JMenuItem mi(String t) {
        ...
        i.setBackground(BG_CARD);
        i.setForeground(WHITE);
        i.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        return i;
    }

    private Component vs(int h) {
